import { Inject, Injectable } from '@angular/core';
import { RoomList } from '../Rooms.model';
import { APP_SERVICE_CONFIG } from 'src/app/AppConfig/appconfig.service';
import { AppConfig } from 'src/app/AppConfig/appconfig.model';
import { HttpClient, HttpHeaders, HttpRequest } from '@angular/common/http';
import { shareReplay } from 'rxjs';
@Injectable({
  providedIn: 'root'
})
export class RoomsService {

//  roomList:RoomList[]=[
//     {
//       roomNumber:1,
//       roomType: 'Deluxe Room',
//       amenities: 'Air Conditionar, Free Wi-Fi',
//       price: 500,
//       photos:
//         'https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.usatoday.com%2Fstory%2Ftravel%2Fhotels%2F2014%2F08%2F14%2Fhotel-booking-tips%2F14006883%2F&psig=AOvVaw1OTh-8ptouOTrfBPkczuYZ&ust=1702474698278000&source=images&cd=vfe&ved=0CBIQjRxqFwoTCJjotM6CioMDFQAAAAAdAAAAABAE',
//         checkinTime:new Date('11-Nov-2021'),
//         checkoutTime:new Date('12-Nov-2021'),
//         rating:4.5,
//     },
//     {
//       roomNumber:2,
//       roomType: 'Deluxe Room',
//       amenities: 'Air Conditionar, Free Wi-Fi',
//       price: 1000,
//       photos:
//         'data:image/jpeg;base64,',
//         checkinTime:new Date('14-Nov-2021'),
//         checkoutTime:new Date('15-Nov-2021'),
//         rating:3.476576,
//     },
//     {
//       roomNumber:3,
//       roomType: 'Deluxe Room',
//       amenities: 'Air Conditionar, Free Wi-Fi',
//       price: 1500,
//       photos:
//         'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRLvsc0vjzXvVSqLVzuIZ5uy9kEhbgAapiHYRXvjTHi_8s6xwbqwZ-B5H55p2-yv6eiaZE&usqp=CAU',
//         checkinTime:new Date('14-Nov-2021'),
//         checkoutTime:new Date('15-Nov-2021'),
//         rating:2.6,
//     },
//   ];
roomList:RoomList[]=[];

// headers=new HttpHeaders({'token':'12askl;gif'})

getRooms$=this.httpClient.get<RoomList[]>('http://localhost:3000/api/rooms').pipe(
  shareReplay(1)
)

  constructor(
    // @Inject(APP_SERVICE_CONFIG)private config:AppConfig,
    private httpClient:HttpClient
  ) {
    // console.log(this.config.apiEndpoint)
    console.log('room service initialization')
   }

  getRooms(){
    // return this.roomList
    // console.log('called')
    
    return this.httpClient.get<RoomList[]>('http://localhost:3000/api/rooms')
  }

  addRoom(room:RoomList){
    return this.httpClient.post<RoomList[]>('http://localhost:3000/api/rooms',room)
  }

  editRoom(room:RoomList){
    return this.httpClient.put<RoomList[]>(`http://localhost:3000/api/rooms/${room.roomNumber}`,room)
  }
  delete(id:string){
    return this.httpClient.delete<RoomList[]>(`http://localhost:3000/api/rooms/${id}`)
  }

  getPhotos(){
    const request =new HttpRequest('GET',`https://jsonplaceholder.typicode.com/photos`,{
      reportProgress:true
    })
    return this.httpClient.request(request)
  }
}
